# Backend Codex Orchestration

This file defines the project-local Codex agent workflow for this Symfony backend.
...
Use `.codex/context/*` as the first-pass memory layer for new sessions.
...
`.codex` is the primary authored workflow layer for this repository.

- Write context only in `.codex/context`
- Update `.codex` workflow files first
- Mirror relevant workflow files into `.claude` after meaningful workflow changes

Do not let `.codex` and `.claude` drift silently.
